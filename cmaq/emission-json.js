...
const createDateString = (day) => {
  ...
  const date = new Date(YEAR, 0, day, 7); // Set hour to 7 to avoid timezone issues
...
async function writeJson(filePrefix, folder, specy) {
  for (const col of [specy]) {
    for (let day = 1; day <= 365; day++) {
      console.log(`  Day ${day}...`);
      const dayStr = createDateString(day);
      const outputDir = join(__dirname, "output", folder, dayStr);
      fs.mkdirSync(outputDir, { recursive: true });
      const fileName = join(outputDir, `${filePrefix}-${col}.json`);
      console.log(`Processing ${col} ${dayStr} -> ${fileName}.`);
        ...
        for (let rowIdx = 0; rowIdx <= 78; rowIdx++) {
          for (let colIdx = 0; colIdx <= 126; colIdx++) {
            ...
            const item = {
              ...
              "row": rowIdx,
              "col": colIdx,
              "value": getSpecyValue(col, rowIdx, colIdx, hour, day)
            };
...
if (require.main === module) {
  (async () => {
    const specy = process.argv[2];
    console.log("Specy:", specy);
    try {
      await residential();
      await strawDx();
      await strawHt();
      ...
      await writeJson("web", "total2", specy);
    } catch (error) {
