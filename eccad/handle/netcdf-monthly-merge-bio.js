...
const CONFIG = {
  ...
  WEB_DATA_FOLDER: '/mnt/disk1/aiotlab/trieutq/inest/fimi-inest-backend/src/scripts/cmaq/output/total2',
  WEB_FILE_PATTERN: '{date}/web-{spec}.json',
  POLLUTANT_MAPPING_FILE: join(__dirname, 'config/mapping_pollutant_to_cmaq_species_BIO.csv'),
  MAP_FACTOR_FILE: join(__dirname, 'resources/hanoi-percentages-2.json'),
  OUTPUT_DIR: join(__dirname, '../output'),
  FILE_PREFIX: 'bio2-merge',
  OUTPUT_FOLDER: 'bio2-merge',
  CELL_FACTOR: 1_000_000 * 1_000_000 / 24 / 3600,
};
...
function processData(fileParam = {}) {
      ...
      data.forEach(({ col, row, time, value }) => {
        ...
        fields.forEach(({ species, ratio }) => {
          if (speciesValue[species] !== undefined) {
            speciesValue[species] += value * ratio; // Nhân hệ số từ cấu hình CSV
          }
        });
      });
  ...
  dataMap.forEach((value, key) => {
    speciesList.forEach(species => {
      value[species] *= CONFIG.CELL_FACTOR; // Nhân hệ số chuyển đổi từ 1 tháng về 1 ngày kèm chuyển đổi đơn vị
    });
  });
}

function loadWebDayMap(dateStr) {
  const map = new Map();
  for (let species of speciesList) {
    const fileName = join(CONFIG.WEB_DATA_FOLDER, format(CONFIG.WEB_FILE_PATTERN, { date: dateStr, spec: species }));
    if (!fs.existsSync(fileName)) {
      console.warn(`Web data file not found: ${fileName}`);
      continue;
    }
    const data = JSON.parse(fs.readFileSync(fileName, 'utf8'));
    data.forEach(({ col, row, hour, value }) => {
      const key = `${row}:${col}:${hour}`;
      if (!map.has(key)) {
        map.set(key, {});
      }
      map.get(key)[species] = value;
    });
  }
  return map;
}
...
async function writeCsvForDay(day, filePrefix, folder) {
  const webDayMap = loadWebDayMap(createDateString(day));
      ...
      for (let tstep = 0; tstep < 25; tstep++) {
        for (let rowIdx = 0; rowIdx < CONFIG.NROWS; rowIdx++) {
          for (let colIdx = 0; colIdx < CONFIG.NCOLS; colIdx++) {
            ...
            // Apply conversion factors
            const excludeHanoiFactor = excludeHanoiFactorMap.get(`${rowIdx}:${colIdx}`) ?? 1;
            if (excludeHanoiFactor !== 1) {
              const webData = webDayMap.get(`${rowIdx}:${colIdx}:${hour}`);
              speciesList.forEach(species => {
                if (data && data[species] !== undefined && webData && webData[species] !== undefined) {
                  data[species] = (data[species] * excludeHanoiFactor) + (webData[species] * (1 - excludeHanoiFactor));
                }
                else {
                  // console.warn(`Missing data for ${rowIdx},${colIdx} species ${species} on day ${day} hour ${hour}`);
                }
              });
            }
